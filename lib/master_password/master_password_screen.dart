...
import '../drive_upload_import/drive_service.dart';
...
class MyDialog {
                      ...
                      TextButton(
                        onPressed: () async {
                          FirebaseDatabase.instance
                            ...
                            getMasterPassword =
                                decryptData(values['masterPassword']);
                                  ...
                                  child: ListBody(
                                    children: <Widget>[
                                      Expanded(
                                        child: CustomBoxTextFormField(
                                          controller:
                                              generateMasterPasswordController,
                                          onChanged: (val) async {},
                                          maxLength: 8,
                                          minLines: 1,
                                          borderRadius: const BorderRadius.only(
                                    ...
                                    children: [
                                      ...
                                      TextButton(
                                        onPressed: () async {
                                          ...
                                          } else if (generateMasterPasswordController
                                                  .text.length <
                                              8) {
                                            Helper.showToast(
                                                "Please enter 8 characters password");
  ...
  ///changes in exportCsvFile function
  Future<Map<String, String?>> exportCSVFile(BuildContext context) async {
    ...
    // After writing the CSV data to the file, call uploadDrive() to upload it to Google Drive
    String? fileId = await uploadDrive(context, path);
    if (fileId != null) {
  ...
  Future<String?> uploadDrive(BuildContext context, String filePath) async {
    ...
    fileId = await _driveService.uploadFile("tasks.csv", filePath);
  ...
  Future<void> downloadCsvFileFromDrive(String fileId) async  {
    ...
    // Call the downloadFile function to download the file from Google Drive
  ...
  void addDataIntoTransactionTable(BuildContext context) async {
    for (int i = 1; i < data.length; i++) {
      ...
      int catId = await DatabaseHelper()
      ...
      TransactionModel transactionModel = TransactionModel(
        ...
        cat_icon: "ic_salary",
        payment_method_id: data[i][5] == "Cash"
            ? 1
            : data[i][5] == "Online"
                ? 2
                : data[i][5] == "Card"
